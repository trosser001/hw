...
#  Lastly, I used 
def count_unique_words(file_name_and_location): # function to perform cleansing and count of unique words in file passed to the function
	...
	remove_remove_remove=str.maketrans('', '',just_delete_these) #  this uses the native string related class in python3 - 'str', vs the 'string' class I used above
	...
	try: 
		with open(file_name_and_location, "r") as file: # "with" is a context manager thing - it manages closing the file if anything goes wrong.
			...
			for line in file:
				...
				cleanline=line.translate(remove_remove_remove) #  using string predefined things (puncuation marks and numbers, plus my )
				...
				unique_words_to_count.update(list_of_words) #  this 'update' puts each word in the list of words, into the set "unique_words_to_count".  The set magically deletes all duplicates.
                
		return len(unique_words_to_count) #  returns the count of unique cleaned words.
    
	except FileNotFoundError: 
		print(f"PROBLEM: File '{file_name_and_location}' was not found") 
	except PermissionError: 
		print(f"PROBLEM: you do not have permission to access the '{file_name_and_location}' file") 
	except UnicodeDecodeError: 
		print(f"PROBLEM: The encoding is not UTF-8 for this file: '{file_name_and_location}'")
	except Exception as e:
		print(f"Problem: Unexpected error occured: {e}")
